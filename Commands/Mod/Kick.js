        ...
        if (AllowGuilds === false) {
            ...
            if (message.guild.id !== AllowedGuildIDs) {
                ...
                if (user1) {
                    ...
                    // Checks If the user is higher rank than bot
                    const highrolekickembed = new Discord.MessageEmbed()
                        .setDescription("*Você não pode expulsar um membro que é superior a você, bobão!*")
                        .setColor(0x36393E)
                        .setTimestamp(Date.now());
                    if (message.member.roles.highest.position <= member.roles.highest.position) return message.channel.send(highrolekickembed);
                    // If the member is in the guild
                    if (member) {
                        ...
                        member
                            ...
                            .then(() => {
                                // We let the message author know we were able to kick the person
                                const kickedembed = new Discord.MessageEmbed()
                                    .setDescription(`***Membro expulso com sucesso! ${user1.tag}***`)
                                    .setColor(0x36393E)
                                    .setTimestamp(Date.now());
                                message.channel.send(kickedembed);
                            })
                            .catch(err => {
                                message.reply('Eu não posso banir esse membro poderoso... (Trombone triste)0');
                                // Log the error
                                console.error(err);
                            });
                    } else {
